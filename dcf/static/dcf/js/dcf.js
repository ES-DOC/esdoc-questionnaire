...
function initializeTag(tag) {
    ...
    var isDefaultCategory = category["default"].toString()=="true"
...
/* copies the (JSON) data - in most cases the global categories dictionary -
 * to a field - in most cases the otherwise unused "expandedTags" CharField -
...
function enable(source,targets) {
    // TODO: HANDLE CASES WHERE SOURCE IS NOT A CHECKBOX?
    var sourceValue = $(source).is(":checked")
    for (var i = 0; i < targets.length; i++) {
        ...
        if (targetType=="button") {
            if (sourceValue) $(target).button("enable");
...
function enableJQueryWidgets() {
    $(function() {
        ...
        /* (the msg dialog box is explicitly setup in the template) */
        $("#help-dialog").dialog({
        ...
        $("button.customize-subform").button({
           ...
           text  : true
            ...
            alert("you clicked subform");
        });
        $("button.customize-subform").button("disable");
        ...
        */
        ...
        $("input,select,textarea").bind("change",function(){
            existingTitle = $("title").text().trim();
            newTitle = existingTitle.replace("*","") + "*";
            $("title").text(newTitle);
        });
