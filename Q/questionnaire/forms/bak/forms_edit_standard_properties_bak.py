...
from Q.questionnaire.q_fields_bak import MetadataFieldTypes, MetadataAtomicFieldTypes, CachedModelChoiceField
from Q.questionnaire.q_fields_bak import METADATA_ATOMICFIELD_MAP, EMPTY_CHOICE, NULL_CHOICE, OTHER_CHOICE
from Q.questionnaire.forms.bak.forms_edit_bak import MetadataEditingForm, MetadataEditingInlineFormSet
from Q.questionnaire.forms.bak.forms_edit_bak import save_valid_subforms
from Q.questionnaire.models.models_customizations import get_existing_customization_set
from Q.questionnaire.models.models_realizations_bak import MetadataModel, MetadataStandardProperty, get_new_realization_set, get_existing_realization_set, get_new_subrealization_set, get_existing_subrealization_set
from Q.questionnaire.models.models_proxies import QStandardPropertyProxy as MetadataStandardPropertyProxy
from Q.questionnaire.models.models_vocabularies import QVocabulary as MetadataVocabulary
...
from Q.questionnaire.views.views_inheritance_bak import set_cached_inheritance_data
from Q.questionnaire.q_utils import QError,find_in_sequence, update_field_widget_attributes, set_field_widget_attributes, get_joined_keys_dict
from Q.questionnaire.forms.bak.forms_base_bak import get_initial_data, model_to_data
...
def create_standard_property_form_data(model, standard_property, standard_property_customizer=None):
    ...
    if standard_property_customizer:
        ...
        assert standard_property_customizer.name == standard_property.name, "%s doesn't equal %s" % (standard_property_customizer.name, standard_property.name)

        field_type = standard_property_form_data["field_type"]

        if field_type == MetadataFieldTypes.ATOMIC:
            value_field_name = "atomic_value"
            if standard_property_customizer.atomic_default:
                standard_property_form_data[value_field_name] = standard_property_customizer.atomic_default
...
class MetadataAbstractStandardPropertyForm(MetadataEditingForm):
    ...
    def customize(self, customizer, **kwargs):
        ...
        from .forms_edit_bak import create_new_edit_subforms_from_models, create_existing_edit_subforms_from_models, create_edit_subforms_from_data
        ...
        if customizer.field_type == MetadataFieldTypes.ATOMIC:
            ...
            if not customizer.relationship_show_subform:
                ...
                reference_field.set_cardinality(customizer.cardinality)
                ...
                subform_customizer = customizer.relationship_subform_customization

                customization_set = get_existing_customization_set(
                    project=subform_customizer.project,
                    ontology=subform_customizer.ontology,
                    proxy=subform_customizer.proxy,
                    customization_id=subform_customizer.pk,
                )
                ...
                scientific_property_customizers = get_joined_keys_dict(customization_set["scientific_property_customizers"])
                ...
                subform_min = int(customizer.get_cardinality_min())
                subform_max = customizer.get_cardinality_max()
                if subform_max != "*":
                    subform_max = int(subform_max)
                render_as_formset = subform_max == "*" or subform_max > 1
                ...
                if property.pk:
                    ...
                    if models:
                        subrealization_set = \
                            get_existing_subrealization_set(
                                models, customization_set["model_customizer"]
                            )

                        get_rid_of_non_displayed_subitems(subrealization_set, {}, customization_set)

                        if not self.is_bound:
                            (model_formset, standard_properties_formsets, scientific_properties_formsets) = \
                                create_existing_edit_subforms_from_models(
                                    subrealization_set["models"], customization_set["model_customizer"],
                                    subrealization_set["standard_properties"], customization_set["standard_property_customizers"],
                                    subrealization_set["scientific_properties"], scientific_property_customizers,
                                    subform_prefix=self.prefix, subform_min=subform_min, subform_max=subform_max,
                                    inheritance_data=inheritance_data
                                )
                        else:
                            (validity, model_formset, standard_properties_formsets, scientific_properties_formsets) = \
                                create_edit_subforms_from_data(self.data,
                                   subrealization_set["models"], customization_set["model_customizer"],
                                   subrealization_set["standard_properties"], customization_set["standard_property_customizers"],
                                   subrealization_set["scientific_properties"], scientific_property_customizers,
                                   subform_prefix=self.prefix, subform_min=subform_min, subform_max=subform_max,
                        ...
                        subrealization_set = \
                            get_new_subrealization_set(subform_customizer.project, subform_customizer.ontology, subform_customizer.proxy, standard_property_proxies, scientific_property_proxies, customization_set["model_customizer"], MetadataVocabulary.objects.none(), self.parent.vocabulary_key, self.parent.component_key )
                    ...
                    if not self.is_bound:
                        (model_formset, standard_properties_formsets, scientific_properties_formsets) = \
                            create_new_edit_subforms_from_models(
                                subrealization_set["models"], customization_set["model_customizer"],
                                subrealization_set["standard_properties"], customization_set["standard_property_customizers"],
                                subrealization_set["scientific_properties"], customization_set["scientific_property_customizers"],
                                subform_prefix=self.prefix, subform_min=subform_min, subform_max=subform_max,
                                inheritance_data=inheritance_data
                            )
                        ...
                        (validity, model_formset, standard_properties_formsets, scientific_properties_formsets) = \
                            create_edit_subforms_from_data(
                                self.data,
                                subrealization_set["models"], customization_set["model_customizer"],
                                subrealization_set["standard_properties"], customization_set["standard_property_customizers"],
                                subrealization_set["scientific_properties"], customization_set["scientific_property_customizers"],
        ...
        if customizer.atomic_suggestions:
            update_field_widget_attributes(value_field, {"class": "autocomplete"})
            update_field_widget_attributes(value_field, {"suggestions": customizer.atomic_suggestions})
