...
class QModelCustomizationForm(QCustomizationForm):
    ...
    _document_fields = ["model_title", "model_description", "model_hierarchy_title", "model_show_empty_categories"]
    _other_fields = []
    ...
    def document_fields(self):
        ...
        :return:
        """
        hierarchical_fields = ["model_hierarchy_title"]
        if self.instance.proxy.has_hierarchical_properties:
            return self.get_fields_from_list(self._document_fields)
        else:
            return self.get_fields_from_list([df for df in self._document_fields if df not in hierarchical_fields])
