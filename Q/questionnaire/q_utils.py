...
class ValidateNoBadSuggestionChars(QValidator):
    ...
    msg = u"Value may not contain any of the following characters: '%s'." % BAD_SUGGESTION_CHARS_LIST

    def __call__(self, value):
        if re.search(BAD_SUGGESTION_CHARS_REGEX, value):
            raise ValidationError(self.msg)
...
def sort_list_by_key(list, key_name, reverse=False):
    ...
    def _sorting_fn(item):
        # using this fn ensures that 'sort_list_by_key' will work
        # for a list of dictionaries (the usual use-case)
        # or a list of objects (a special use-case; a QS can use the '.order_by' filter, but an actual list of models cannot)
        try:
...
def copy_model(model, fields={}):
    ...
    (used mainly in "models_customizations#get_new_customizations"
    to ensure that each CIM model property has a unique customization)
    ...
    for field_name, field_value in fields.iteritems():
        setattr(new_model, field_name, field_value)
        # AS OF v0.16.0.0, CALLS TO "copy_model" ARE WRAPPED W/ "allow_unsaved_fk" CONTEXT MANAGER AS NEEDED
        # if not isinstance(new_model.get_field(field_name), ForeignKey):
        #     setattr(new_model, field_name, field_value)
        # else:
        #     # if this is a fk, there is a chance that it may not have been saved yet..
        #     with allow_unsaved_fk(type(new_model), [field_name, ]):
        #         setattr(new_model, field_name, field_value)
...
def get_key_from_value(dct, value):
    """
    given a value, returns the key of the (1st matching) value in the dict
    :param dct:  dict to check
    ...
    return None
    # try:
    # NOT SURE IF 'keys()' & 'values()' RETURN THINGS IN THE SAME ORDER
    #     value_index = dct.values().index(value)
    #     return dct.keys()[value_index]
    # except ValueError:
    #     # value was not in dct
    #     return None
