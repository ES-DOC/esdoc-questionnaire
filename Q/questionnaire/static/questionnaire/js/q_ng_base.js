...
(function() {
    ...
    /* note my use of angular-loading-bar */
    ...
    */
    ...
    app.directive('subformSection', ['$http', '$compile', function($http, $compile) {
        return {
            ...
            link: function (scope, element, attrs) {
                /* note that ng converts everything to camelCase */
                var model_scope = attrs["scope"];
                var watched_name = model_scope + ".display_subform_detail";
                ...
                scope.$watch(watched_name, function (is_displayed) {
                    if (is_displayed && !$(element).hasClass("loaded")) {
                        ...
                        var data = "session_key=" + session_key + "&scope=" + model_scope + "&key=" + attrs["key"];
                        $http({
                            method: "POST",
                            url: url,
                            headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                            ...
                            function() {
                                $(element).addClass("loaded")
                                $(element).trigger("loaded_subform")
                            }
                        );
                    }
