...
class QProject(models.Model):
    ...
    logo = models.ImageField(upload_to=generate_upload_to, blank=True, max_length=None, storage=OverwriteStorage())
    ...
    vocabularies = models.ManyToManyField(
        "QVocabulary",
        blank=True,
        through="QProjectVocabulary",  # note my use of the "through" model
        help_text="Only registered vocabularies can be added to projects."
    )

    def __unicode__(self):
        return u"QProject: %s" % (self.name)
...
class QProjectVocabulary(models.Model):
    class Meta:
        app_label = APP_LABEL
        abstract = False
