...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'MetadataVersion', fields ['name']
        db.delete_unique(u'questionnaire_metadataversion', ['name'])

        # Adding field 'MetadataVersion.version'
        db.add_column(u'questionnaire_metadataversion', 'version',
                      self.gf('django.db.models.fields.CharField')(max_length=256, null=True, blank=True),
                      keep_default=False)

        # Adding unique constraint on 'MetadataVersion', fields ['name', 'version']
        db.create_unique(u'questionnaire_metadataversion', ['name', 'version'])


    def backwards(self, orm):
        # Removing unique constraint on 'MetadataVersion', fields ['name', 'version']
        db.delete_unique(u'questionnaire_metadataversion', ['name', 'version'])

        # Deleting field 'MetadataVersion.version'
        db.delete_column(u'questionnaire_metadataversion', 'version')

        # Adding unique constraint on 'MetadataVersion', fields ['name']
        db.create_unique(u'questionnaire_metadataversion', ['name'])
