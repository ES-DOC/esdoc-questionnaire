...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'MetadataModelCustomizerVocabulary'
        db.create_table(u'questionnaire_metadatamodelcustomizervocabulary', (
            (u'id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('model_customizer', self.gf('django.db.models.fields.related.ForeignKey')(related_name='model_customizer', to=orm['questionnaire.MetadataModelCustomizer'])),
        ...
        db.send_create_signal('questionnaire', ['MetadataModelCustomizerVocabulary'])

        # Adding unique constraint on 'MetadataModelCustomizerVocabulary', fields ['model_customizer', 'vocabulary']
        db.create_unique(u'questionnaire_metadatamodelcustomizervocabulary', ['model_customizer_id', 'vocabulary_id'])
        ...
        MetadataModelCustomizerClass = ContentType.objects.get(app_label="questionnaire", model="metadatamodelcustomizer").model_class()
        MetadataVocabularyClass = ContentType.objects.get(app_label="questionnaire", model="metadatavocabulary").model_class()
        MetadataModelCustomizerVocabularyClass = ContentType.objects.get(app_label="questionnaire", model="metadatamodelcustomizervocabulary").model_class()

        for metadata_model_customizer in MetadataModelCustomizerClass.objects.all():
            vocabulary_order = metadata_model_customizer.vocabulary_order.split(',')
            for (i, metadata_vocabulary) in enumerate(metadata_model_customizer.vocabularies.all()):
                ...
                if vocabulary_pk in vocabulary_order:
                    metadata_model_customizer_vocabulary.active = True
                metadata_model_customizer_vocabulary.order = i  # too complex to work out order, just cope w/ resetting it later
                metadata_model_customizer_vocabulary.save()
        # END MODIFIED BY AT
    ...
    def backwards(self, orm):
        ...
        # (see http://stackoverflow.com/questions/7878605/how-to-migrate-with-south-when-using-through-for-a-manytomany-field)
        MetadataModelCustomizerVocabularyClass = ContentType.objects.get(app_label="questionnaire", model="metadatamodelcustomizervocabulary").model_class()
        for metadata_model_customizer_vocabulary in MetadataModelCustomizerVocabularyClass.objects.all():
            metadata_model_customizer = metadata_model_customizer_vocabulary.model_customizer
            metadata_vocabulary = metadata_model_customizer_vocabulary.vocabulary
            existing_vocabularies = metadata_model_customizer.vocabularies.all()
            existing_vocabulary_order = metadata_model_customizer.vocabulary_order.split(',')
            vocabulary_pk = u"%s" % metadata_vocabulary.pk
            if metadata_vocabulary not in existing_vocabularies:
                metadata_model_customizer.vocabularies.add(metadata_vocabulary)
            if metadata_model_customizer_vocabulary.active and vocabulary_pk not in existing_vocabulary_order:
                existing_vocabulary_order.append(vocabulary_pk)  # again, don't worry about precise ordering
            metadata_model_customizer.save()
        # END MODIFIED BY AT

        # Removing unique constraint on 'MetadataModelCustomizerVocabulary', fields ['model_customizer', 'vocabulary']
        db.delete_unique(u'questionnaire_metadatamodelcustomizervocabulary', ['model_customizer_id', 'vocabulary_id'])

        # Deleting model 'MetadataModelCustomizerVocabulary'
        db.delete_table(u'questionnaire_metadatamodelcustomizervocabulary')
    ...
    models = {
        ...
        'questionnaire.metadatamodelcustomizervocabulary': {
            'Meta': {'ordering': "('order',)", 'unique_together': "(('model_customizer', 'vocabulary'),)", 'object_name': 'MetadataModelCustomizerVocabulary'},
            'active': ('django.db.models.fields.BooleanField', [], {'default': 'False'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'model_customizer': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'model_customizer'", 'to': "orm['questionnaire.MetadataModelCustomizer']"}),
        ...
        'questionnaire.metadatastandardpropertycustomizer': {
            ...
            'atomic_type': ('django.db.models.fields.CharField', [], {'default': "'DEFAULT'", 'max_length': '512'}),
