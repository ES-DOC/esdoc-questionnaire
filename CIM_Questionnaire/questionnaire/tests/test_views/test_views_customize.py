...
class Test(TestQuestionnaireBase):
    ...
    def test_validate_view_arguments(self):
        ...
        # version is "uncategorized"
        invalid_kwargs = kwargs.copy()
        categorization = self.version.categorization
        self.version.categorization = None
        self.version.save()
        (validity,project,version,model_proxy,msg) = validate_view_arguments(**invalid_kwargs)
        self.assertEqual(validity,False)
        self.version.categorization = categorization
        self.version.save()
    ...
    def test_questionnaire_customize_help_get(self):
        ...
        with self.assertNumQueries(query_limit):
            request_url = reverse("customize_help")
    ...
    #     vocabularies_to_customize = self.project.vocabularies.filter(document_type__iexact=self.model_customizer.proxy.name)
    ...
    #
    #     existing_model_customizer = self.model_customizer
    #
    ...
    #     vocabularies_to_customize = self.project.vocabularies.filter(document_type__iexact=self.model_customizer.proxy.name)
    #     (model_customizer,standard_category_customizers, standard_property_customizers, scientific_category_customizers, scientific_property_customizers) = \
    #         MetadataCustomizer.get_existing_customizer_set(self.model_customizer, vocabularies_to_customize)
    #
    #     (model_customizer_form, standard_property_customizer_formset, scientific_property_customizer_formsets) = \
